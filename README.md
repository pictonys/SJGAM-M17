...
Тут находится всё, что я знаю об данной бюджетной, в полне неплохой, если её доработать, консоли - SJGAM M17 ревизии W12-V04.
...
## Технические характеристики консоли и не только:

- **SoC:** Rockchip RK3126C (форм-фактор LQFP) - четырехядерный ARM Cortex-A7 с частотой до 1.2 GHz (по стоку в device-tree он только на 1.2 GHz и работает) - видеоядро Mali-400 MP2 (по стоку в device-tree он работает только в 480 МГц).
- **Оперативная память:** 256MB DDR3 SDRAM - Samsung K4B1G0846F-HCH9 (форм-фактор FBGA78) - 2 чипа по 128MB каждый, работающих в 480 МГц (вплоть до 667 МГц, но напряжения не хватает, консоль зависает при насильном выставлении МГц).
- **Встроенная память (eMMC NAND Flash):** Toshiba/Kioxia THGBMDG5D1LBAIL (форм-фактор WFBGA153) - 4 GB, интерфейс eMMC 5.0.
- **Дисплейный модуль:** GV043WXY002LA0703SW с тонированым стеклом (вероятно, сделанная на заказ) - 40 pin FPC - 480x272 - 4.3", соотношение сторон - 16:9.06.
- **Аккумулятор:** Li-Polymer LB 405267 - 2000mAh - 3.85V - 7.7Wh.
- **Контроллер питания:** Микросхема линейного зарядного устройства для одного Li-Ion/Li-Polymer аккумулятора - Nanjing Micro One Electronics Inc. (Microne) ME4057D (форм-фактор SOP-8).
- **Аудиоусилитель:** Монофонический звуковой усилитель мощности класса D/F - Shanghai Mixinno Microelectronics Co., Ltd. (Mixinno) MIX2018 (форм-фактор ESOP-8).
- **Порты:** USB Type-C (OTG включен в device-tree (на uboot и на boot), но на порт не разведен), TRRS 3.5mm (Mini Jack в простонародии, вывод звука и вход наушников (с микрофоном или без) и слот microSDXC (пока не известен макс. объем, который эта консоль примет. Теоретически - 2 ТБ, так как Synopsys DesignWare Mobile Storage Host Controller в SoC поддерживает адресацию стандарта SD 3.0 (и не только).
- **Управление:** механическое - низкопрофильные тактовые SMD кнопки громкости + / - и Start / Select, абсолютно обычный тумблер состояния консоли On / Off, два аналоговых стика (подделка на стики Nintendo Switch, только в оригинале они на шлейфе, а в нашей подделке они на пинах) с кнопками R3 / L3, тактовые кнопки с резиновым актуатором (шифты/курки) L2 / R2 и тактовые кнопки с монтажом под прямым углом (шифты/курки) L1 / R1, а также токопроводящие кнопки (мембраны) D-Pad (UP - DOWN - LEFT - RIGHT) и родные Xbox'овские A / B / Y / X.
- **Вывод звука:** 1 плёночный динамик с неизвестными характеристиками, по моему 8 Ом.
- **I²C контроллер:** проприетарный NoName без маркировки.
- **Коннекторы внутри:** FPC / FFC ZIF 40 pin для матрицы, два FPC / FFC ZIF 18 pin для соединения платы кнопок, стика и светодиодов справа с материнской платой с соответствующий шлейфом 18 pin, JST-EL типа коннектор для аккумулятора, JST-SH типа коннектор для динамика.

- Стоит учесть, что на плате также находится место распайки UART порта и SPI флешки.
...
- - trust: содержит разные ключи и подписи.
- - uboot: отвечает за инициализацию начинки (RAM, eMMC и т.п.). Имеет device-tree, в котором редактированию ничего не подлежит.
...
- - recovery: содержит всё, что в boot, содержит device-tree, но и имеет ramdisk с "левым" buildroot, который запускает только em_ui.sh с флешки, не используется, само рекавери никак не запускается, тоже просто ради "галочки".
...
- boot: бут как в Android, содержит device-tree, который уже можно и редактировать, ramdisk отсутствует. Грузит систему напрямую (rootfs) по UUID раздела.

- rootfs: содержит систему Linux/Buildroot, собранную через SDK Rockchip Buildroot 2018.02-rc3-01346-gd8cef2a-dirty. SquashFS. Логика запуска в /etc/init.d/. Если вставлена карта памяти, то при запуске - скрипт инита buildroot'а запустит с нее em_ui.sh скрипт, за место встроенного /usr/bin/game (minigui) (китайский GUI с играми), в качестве эмулятора в данном minigui выступает китайский кастрированый билд RetroArch.

- userdata: r/w, в него скидываются всякие конфиги ретроарча с rootfs, без этого раздела запуска не будет.

## Режим "LOADER"

Вход осуществляется следующим образом: выключаем консоль, вставляем USB Type-C кабель в консоль и в ПК/Ноутбук, зажимаем кнопку громкости **-** на консоли и включаем её. 

## Режим "MASKROM"

Вход осуществляется двумя способами: 

- 1: Выключаем консоль, разбираем, отключаем аккумулятор и динамик, переводим выключатель в позицию **On**, вставляем USB Type-C кабель в консоль, разьем USB держим возле ПК/Ноутбука (не втыкаем), замыкаем [Test Point](Photos/testpoint.png) и втыкаем кабель. Вы в MASKROM. Также, как я, можно вывести кнопку для MASKROM (убедитесь, что кнопка имеет сопротивление в районе 0.1–1 Ома и имелись максимально короткие провода. Выбирайте места рядом с тест поинтом. Например под шифтами/курками L).
...
```
Да, тулчейн староват, но сборка программ и нахождение собранных тем же тулчейном программ - возможна.
...
Собрано сносно. См. [deconfig](https://github.com/pictonys/SJGAM-M17/blob/main/Stock%20Kernel%20Deconfig%20(4.4.179)/M17_W12-V04_KERNEL_DECONFIG), вытащенный через extract-ikconfig.

## Состояние древа:

Собрано вполне неплохо, но есть неприятные моменты в виде работы процессора только на одной частоте.
