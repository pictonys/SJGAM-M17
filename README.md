...
Вход осуществляется так: выключаем консоль, разбираем, отключаем аккумулятор и динамик, переводим выключатель в позицию **On**, вставляем USB Type-C кабель в консоль, разьем USB держим возле ПК/Ноутбука (не втыкаем пока что), замыкаем 
